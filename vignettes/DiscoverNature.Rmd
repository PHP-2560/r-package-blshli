...
```
This code should provide access to the database with information to perform a basic search. There are three main categories that can be searched using this database. These are: **explore_keyword( )**, **explore_subject( )****/****list.topics( )**, and **explore_paper( )**. 

## Using explore_keyword
One of the options to search the database is through keywords. 
```{r }
paper.results <- explore_keyword('cells', u.year = 1846 )
paper.results 
```
...
## Using explore_paper
This functions provides an easy way for the user to open an article of interest in a browser without having to manually identify url of paper. To do this, the user will need to input both a title and result based on a *keyword* or *subject* search as shown in the code.

```{r }
explore_paper(.title = 'The Zephyr', res = paper.results)
...
#paper_by_sub %>%
  #View()
```
The message will suggest use of the option ***list.topics()***. This function provides the user with an up to date list of available topics to choose from. See the code below:
```{r }
#topics <- list.topics()
#topics %>%
  #View()
